...
public class FastisController<Value: FastisValue>: UIViewController, JTACMonthViewDelegate, JTACMonthViewDataSource {
    ...
    open lazy var calendarView: JTACMonthView = {
...
//    }()
    ...
    private let weekHeaderReuseIdentifier = "weekHeaderReuseIdentifier"
    ...
    private func handleDateTap(in calendar: JTACMonthView, date: Date) {
        ...
        case .range:
            ...
            if let currentValue = self.value as? FastisRange {
                ...
                else if date.isInSameDay(in: self.currentCalendar, date: currentValue.fromDate) { //End Date
//                    if currentValue.fromDate.compare(date) == .orderedAscending {
                        let newToDate = date.endOfDay(in: self.currentCalendar)
                        newValue = .from(newToDate, to: newToDate)
//                    }
//                    else {
//                        let newToDate = date.endOfDay(in: self.currentCalendar)
//                        newValue = .from(currentValue.fromDate, to: newToDate)
//                    }
                } else if date.isInSameDay(in: self.currentCalendar, date: currentValue.toDate) { // Start DAte
//                    if currentValue.toDate.compare(date) == .orderedDescending {
                        let newFromDate = date.startOfDay(in: self.currentCalendar)
                        newValue = .from(newFromDate, to: newFromDate)
//                    } else {
//                        let newFromDate = date.startOfDay(in: self.currentCalendar)
//                        newValue = .from(newFromDate, to: currentValue.toDate)
//                    }
                } else if date < currentValue.fromDate { // Start and End date Already selected and select previous date from strat date
                    let newFromDate = date.startOfDay(in: self.currentCalendar)
                    newValue = .from(newFromDate, to: newFromDate)
                } else {
                    if date > currentValue.toDate && currentValue.fromDate != currentValue.toDate { // Start and End date Already selected and select date after end date
                        let newToDate = date.endOfDay(in: self.currentCalendar)
                        newValue = .from(newToDate, to: newToDate)
                    } else if date > currentValue.fromDate && date < currentValue.toDate {//  Start and End date Already selected and select between date
                        let newToDate = date.endOfDay(in: self.currentCalendar)
                        newValue = .from(newToDate, to: newToDate)
                    } else {// If Start date selected and select end date greter then start date
                        let newToDate = date.endOfDay(in: self.currentCalendar)
                        newValue = .from(currentValue.fromDate, to: newToDate)
                    }
                }
            ...
            let nightCount = nightCount(start: newValue.fromDate, end: newValue.toDate)
            if nightCount > 30 {
                self.dismissHandler?()
            } else {
    ...
    private func handleDateTapForDisable(in calendar: JTACMonthView, date: Date) -> Bool {
...
//                    }
                    ...
                    if date > currentValue.toDate && currentValue.fromDate != currentValue.toDate {
                        let newToDate = date.endOfDay(in: self.currentCalendar)
                        newValue = .from(newToDate, to: newToDate)
                    } else {
                        let newToDate = date.endOfDay(in: self.currentCalendar)
    ...
    private func nightCount(start: Date, end: Date) -> Int {
        let dayCount = Calendar.current.dateComponents([.day], from: start, to: end).day!
    ...
    public func configureCalendar(_ calendar: JTACMonthView) -> ConfigurationParameters {
        ...
        if let maximumDate = self.privateMaximumDate,
            let endOfNextMonth = self.currentCalendar.date(byAdding: .month, value: 0, to: maximumDate)?
                .endOfMonth(in: self.currentCalendar) {
            endDate = endOfNextMonth
        }
        
        if let minimumDate = self.privateMinimumDate,
            let startOfPreviousMonth = self.currentCalendar.date(byAdding: .month, value: 0, to: minimumDate)?
    ...
    public func calendar(_ calendar: JTACMonthView, headerViewForDateRange range: (start: Date, end: Date), at indexPath: IndexPath) -> JTACMonthReusableView {
        ...
        if self.privateSelectMonthOnHeaderTap, Value.mode == .range {
            header.tapHandler = {
                ...
                let endDate = Calendar.current.date(byAdding: .day, value: 30, to: fromDate)!
                let newValue: FastisRange = .from(fromDate, to: endDate)
                self.value = newValue as? Value
    ...
    public func calendar(_ calendar: JTACMonthView, shouldSelectDate date: Date, cell: JTACDayCell?, cellState: CellState, indexPath: IndexPath) -> Bool {
        self.viewConfigs.removeAll()
        return self.handleDateTapForDisable(in: calendar, date: date)
    }
...
extension FastisController where Value == FastisRange {
    ...
    public convenience init(mode: FastisModeRange, config: FastisConfig = .default) {
        ...
        self.selectMonthOnHeaderTap = false
    }
